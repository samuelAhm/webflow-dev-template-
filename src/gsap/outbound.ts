...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getNextWeekDateRange() {
    ...
    const monthFormatted = nextMonday.toLocaleDateString('en-US', options);

    // Add ordinal suffixes to the days
    const mondayDay = nextMonday.getDate();
    ...
    const nextWeekDates = [] as string[];
    // Loop to get and format dates for Monday through Friday
    for (let i = 0; i < 5; i++) {
      const date = new Date(today);
      date.setDate(today.getDate() + daysUntilNextMonday + i);
      nextWeekDates.push(date.toLocaleDateString('en-US', secOptions)); // Format date as "Wed, 28 Sep 2024"
    }

    countdownToEventHours(`${nextWeekDates[0]} 00:00:00`);

    // Display each date on a new line
    const formattedDates = nextWeekDates.join('<b>'); // Join with line breakse
